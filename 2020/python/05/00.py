...
def findMissing(array):
    for i in range(len(array)):
        theoreticalNumber = i + array[0]
        if array[i] != theoreticalNumber:
            return array[i]-1 # -1 because the PRIOR seat is missing, not the one, the iterator points to
    return -1
...
sortedSeats = np.sort(seatIDs)

print("searching missing")

print(findMissing(sortedSeats))